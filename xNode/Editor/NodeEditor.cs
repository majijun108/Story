...
namespace XNodeEditor {
    ...
    public class NodeEditor : XNodeEditor.Internal.NodeEditorBase<NodeEditor, NodeEditor.CustomNodeEditorAttribute, XNode.Node> {
        ...
        {
            base.OnCreate();
            var nodeTitleAttribute = target.GetType().GetCustomAttributes(typeof(XNode.Node.NodeTitleAttribute), false);
            if (nodeTitleAttribute != null && nodeTitleAttribute.Length > 0)
            {
                var attrib = nodeTitleAttribute[0] as XNode.Node.NodeTitleAttribute;
                title = attrib.title;
                allowCustomName = attrib.allowCustomName;
            }
        ...
        public virtual void OnHeaderGUI() {
            if (allowCustomName)
                GUILayout.Label($"{title}({target.name})", NodeEditorResources.styles.nodeHeader, GUILayout.Height(30));
            else
